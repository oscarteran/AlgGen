...
def grafResiduales1d(x,y,x1,y1,r,title,xlabel,ylabel):
    ...
    ax.set(title = title, xlabel = xlabel, ylabel = ylabel, xscale = 'linear')
    ax.plot(x, y, marker='v', c = 'r',label = 'Datos Reales')
    ax.plot(x1, y1, marker='.', c = 'b' ,label = 'Datos Invertidos')
...
grafResiduales1d(M[:,0],M[:,1],M1[:,0],M1[:,1],R,'Anomalia Gravimetrica','x[km]','Delta_G [mGal]')
